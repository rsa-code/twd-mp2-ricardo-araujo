...
export const NanaCharacters: React.FC = () => {
  ...
  const highlightedIds = useSelector(
    (state: RootState) => state.highlights.highlightedIds,
  );
  ...
  const others = characters.edges.filter(
    (edge) => !highlightedIds.includes(edge.node.id),
  );
            ...
            {highlighted.map(({ node, role }) => (
              <div key={node.id} className="relative">
                ...
                >
                  <Star
                    size={16}
                    className="text-rose-600 hover:text-white fill-rose-600"
                  />
                </button>
                <CharacterCard
                  character={node}
