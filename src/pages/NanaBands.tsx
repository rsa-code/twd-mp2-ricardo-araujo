...
const BANDS = [
  ...
  },
  {
    id: 'trapnest',
    name: 'Trapnest',
    description: 'A popular rock band that Ren Honjo joined, leaving Blast behind.',
    members: [
        ...
        {BANDS.map((band) => (
          <div key={band.id} className="space-y-12">
              <h2 className="text-4xl md:text-5xl font-black font-cinzel text-white uppercase tracking-tight">
                {band.name}
                {band.alias && <span className="text-zinc-600 ml-4 text-2xl md:text-3xl">{band.alias}</span>}
              </h2>
              <p className="text-zinc-400 text-lg max-w-2xl">{band.description}</p>
              ...
              {band.members.map((member) => {
                ...
                return (
                  <CharacterCard 
                    ...
                    role={member.role} 
                    variant="default"
                    onClick={() => setSelectedCharacter({ node: character, role: member.role })}
                  />
                );
              })}
            </div>
