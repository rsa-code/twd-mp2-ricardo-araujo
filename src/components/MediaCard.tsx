...
export const MediaCard: React.FC<MediaCardProps> = ({
  ...
  return (
    <div
      className={`flex flex-col lg:flex-row gap-8 items-start py-12 ${isLeft ? "" : "lg:flex-row-reverse"}`}
    >
      ...
      <div className="w-full lg:w-2/3 space-y-6">
        <div
          className={`flex flex-col ${isLeft ? "items-start" : "lg:items-end lg:text-right"}`}
        >
          <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-white mb-2">
            {media.type}
          </h2>
          <h3 className="text-xl text-rose-500 font-cinzel italic">
            {media.title.native}
          </h3>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {media.genres.map((genre) => (
            <span
              key={genre}
              className="px-3 py-1 border border-zinc-700 text-xs uppercase tracking-wider text-zinc-400 hover:border-rose-600 hover:text-rose-500 transition-colors"
            >
              {genre}
            </span>
          ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 pt-6 border-t border-zinc-800">
          ...
          <div className="flex flex-col">
            ...
            <span
              className={`font-bold uppercase ${media.status === "FINISHED"}`}
            >
              {media.status}
            </span>
          </div>
